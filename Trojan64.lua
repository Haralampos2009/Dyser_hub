...
local LootMachinesSettings = {
    BlackCase = {En = false, Nm = false, Color = Color3.fromRGB(255, 0, 255)},
    Box = {En = false, Nm = false, Color = Color3.fromRGB(255, 255, 0)},
    Vending = {En = false, Nm = false, Color = Color3.fromRGB(255, 120, 0)},
    Bucket = {En = false, Nm = false, Color = Color3.fromRGB(0, 255, 255)},
    ShowDist = false,
    MaxDist = 1000
}
...
local v0 = loadstring(game:HttpGet("https://raw.githubusercontent.com/Haralampos2009/Dyser_hub/refs/heads/main/1.txt"))();
local l_v0_Window_0 = v0:CreateWindow({
    Name = "\239\191\189\239\191\189\239\191\189\239\191\189\239\191\189 Dyser \239\191\189\239\191\189\239\191\189\239\191\189\239\191\189\239\191\189", 
    Icon = 0, 
    LoadingTitle = "Dyser now loading", 
...
-- Создаем новую вкладку в вашем окне (используем переменную l_v0_Window_0 из вашего файла)
local WorldTab = l_v0_Window_0:CreateTab("World Visuals", 4483362458)
...
-- Кнопка для запуска вашего внешнего скрипта (Deazer-C)
WorldTab:CreateButton({
    Name = "Freecam (Shift + P)",
    Callback = function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/Haralampos2009/Deazer-C/main/3.lua"))()
    end,
})
...
-- Цикл для обновления времени (аналог вашего heartbeat)
spawn(function()
    while task.wait() do
        if timechanger then
...
WorldTab:CreateToggle({
    ...
    Flag = "CupboardToggle", 
    Callback = function(Value)
        LootMachinesSettings.Cupboard.En = Value -- Переключаем состояние
    end,
})
...
task.spawn(function()
    while task.wait(0.5) do
        for _, obj in pairs(workspace:GetDescendants()) do
            -- Проверяем структуру: Model -> Hitbox + Part (как на твоем скрине)
            if obj:IsA("Model") and obj:FindFirstChild("Hitbox") and obj:FindFirstChild("Part") then
                
                -- Если ESP ВКЛЮЧЕН
                if LootMachinesSettings.Cupboard.En then
                    if not obj:FindFirstChild("LM_HL") then
                        -- Создаем подсветку
                        local hl = Instance.new("Highlight", obj)
                        hl.Name = "LM_HL"
                        hl.FillColor = LootMachinesSettings.Cupboard.Color
                        ...
                        lbl.Text = "CUPBOARD"
                        lbl.TextColor3 = LootMachinesSettings.Cupboard.Color
                        lbl.Font = Enum.Font.GothamBold
                        lbl.TextScaled = true
                    end
                else
                    -- Если ESP ВЫКЛЮЧЕН — удаляем всё
                    if obj:FindFirstChild("LM_HL") then obj.LM_HL:Destroy() end
                    if obj:FindFirstChild("LM_Gui") then obj.LM_Gui:Destroy() end
                end
...
local LMSection = WorldTab:CreateSection("Loot & Machines")
...
WorldTab:CreateSlider({
    ...
    Increment = 50,
    CurrentValue = 1000,
    Callback = function(Value) LootMachinesSettings.MaxDist = Value end,
})

WorldTab:CreateToggle({
    Name = "Show Distance on Loot",
    CurrentValue = false,
    Callback = function(Value) LootMachinesSettings.ShowDist = Value end,
})

-- Пример для Черного кейса (остальные делаются так же)
WorldTab:CreateToggle({
    Name = "Black Case ESP",
    CurrentValue = false,
    Callback = function(Value) LootMachinesSettings.BlackCase.En = Value end,
})

-- Кнопка для STORAGE BOX
WorldTab:CreateToggle({
    Name = "Storage Box ESP",
    CurrentValue = false,
    Callback = function(Value) LootMachinesSettings.Box.En = Value end,
})

-- Кнопка для VENDING
WorldTab:CreateToggle({
    Name = "Vending Machine ESP",
    CurrentValue = false,
    Callback = function(Value) LootMachinesSettings.Vending.En = Value end,
})
...
WorldTab:CreateToggle({
    Name = "Bucket ESP",
    CurrentValue = false,
    Callback = function(Value) LootMachinesSettings.Bucket.En = Value end,
})




local function UpdateLM(Obj, Config, Tag, PartName)
    local Part = Obj:FindFirstChild(PartName) or Obj:FindFirstChildOfClass("BasePart")
    if not Part then return end
    local Dist = (camera.CFrame.Position - Part.Position).Magnitude

    if Dist > LootMachinesSettings.MaxDist or not Config.En then
        if Obj:FindFirstChild("LM_HL") then Obj.LM_HL:Destroy() end
        if Obj:FindFirstChild("LM_Gui") then Obj.LM_Gui:Destroy() end
        return
    end
    ...
    -- Подсветка
    local hl = Obj:FindFirstChild("LM_HL") or Instance.new("Highlight", Obj)
    hl.Name = "LM_HL"
    ...
    -- Надпись
    if Config.Nm then
        local gui = Obj:FindFirstChild("LM_Gui") or Instance.new("BillboardGui", Obj)
        gui.Name = "LM_Gui"
        gui.AlwaysOnTop = true
        gui.Size = UDim2.new(0, 50, 0, 30)
        gui.ExtentsOffset = Vector3.new(0, 1, 0)
        local lbl = gui:FindFirstChild("L") or Instance.new("TextLabel", gui)
        lbl.Name = "L"
        lbl.BackgroundTransparency = 1
        lbl.Size = UDim2.new(1, 0, 1, 0)
        lbl.TextColor3 = Config.Color
        lbl.Text = Tag .. (LootMachinesSettings.ShowDist and string.format("\n[%d m]", math.floor(Dist)) or "")
    end
end
    ...
    while task.wait(0.3) do
        for _, v in pairs(workspace:GetChildren()) do
            if v.Name == "Model" then
                if v:FindFirstChild("Body") and v:FindFirstChild("default") then
                    UpdateLM(v, LootMachinesSettings.BlackCase, "BLACK CASE", "Body")
                elseif v:FindFirstChild("box") and v:FindFirstChild("trash") then
                    UpdateLM(v, LootMachinesSettings.Box, "STORAGE BOX", "box")
                elseif v:FindFirstChild("Dispenser") and v:FindFirstChild("Machine") then
                    UpdateLM(v, LootMachinesSettings.Vending, "VENDING", "Machine")
                elseif v:FindFirstChild("default") and v:FindFirstChild("Part") then
                    UpdateLM(v, LootMachinesSettings.Bucket, "BUCKET", "default")
                end
            end
        end
    end
...
-- Секция для тотемов
local TotemSection = WorldTab:CreateSection("Totem ESP")
...
WorldTab:CreateToggle({
    ...
    Flag = "TotemEnabled",
    Callback = function(Value)
        TotemSettings.Enabled = Value
    end,
})

WorldTab:CreateColorPicker({
    Name = "Totem ESP Color",
    Color = TotemSettings.Color,
    Flag = "TotemColor",
    Callback = function(Value)
        TotemSettings.Color = Value
    end,
})

WorldTab:CreateToggle({
    Name = "Show Totem Distance",
    CurrentValue = false,
    Flag = "TotemDist",
    Callback = function(Value)
        TotemSettings.Distance = Value
    end,
})

--- Логика работы ESP (вставляется один раз в конец скрипта) ---
task.spawn(function()
    while task.wait(0.5) do -- Проверка каждые 0.5 сек для экономии ресурсов
        if not TotemSettings.Enabled then
            -- Очистка, если выключено
            for _, v in pairs(workspace:GetChildren()) do
                if v.Name == "Model" and v:FindFirstChild("Base") then
                    if v:FindFirstChild("T_HL") then v.T_HL:Destroy() end
                    if v:FindFirstChild("T_Gui") then v.T_Gui:Destroy() end
                end
            end
            continue
        end
        ...
        for _, v in pairs(workspace:GetChildren()) do
            -- Условие из Trident Survival: модель с Base и Body
            if v.Name == "Model" and v:FindFirstChild("Base") and v:FindFirstChild("Body") then
                
                -- Подсветка
                local hl = v:FindFirstChild("T_HL") or Instance.new("Highlight", v)
                hl.Name = "T_HL"
                hl.FillColor = TotemSettings.Color
                hl.OutlineTransparency = 0
                hl.FillTransparency = 0.5

                -- Текст (BillboardGui)
                local gui = v:FindFirstChild("T_Gui") or Instance.new("BillboardGui", v)
                gui.Name = "T_Gui"
                gui.AlwaysOnTop = true
                gui.Size = UDim2.new(0, 50, 0, 25)
                gui.ExtentsOffset = Vector3.new(0, 1, 0)
                ...
                lbl.Size = UDim2.new(1, 0, 1, 0)
                lbl.Font = Enum.Font.GothamBold
                lbl.TextColor3 = TotemSettings.Color
                lbl.TextScaled = true
                lbl.TextStrokeTransparency = 0

                local text = "TOTEM"
                if TotemSettings.Distance and camera then
                    local dist = math.floor((camera.CFrame.Position - v.Base.Position).Magnitude)
...
-- Создаем новую вкладку Loot ESP (вставляй после создания других вкладок)
local LootSettings = {
    Safe = {En = false, Nm = false, Color = Color3.fromRGB(105, 105, 105)},
    MilCrate = {En = false, Nm = false, Color = Color3.fromRGB(0, 100, 0)},
    MaxDist = 1500
}



local SafeSection = WorldTab:CreateSection("Safe (Сейфы)")

WorldTab:CreateToggle({
    Name = "Enable Safe ESP",
    CurrentValue = false,
    Flag = "SafeEnabled",
    Callback = function(Value) LootSettings.Safe.En = Value end,
})
...
WorldTab:CreateToggle({
    ...
    CurrentValue = false,
    Flag = "SafeName",
    Callback = function(Value) LootSettings.Safe.Nm = Value end,
})

WorldTab:CreateColorPicker({
    Name = "Safe Color",
    Color = LootSettings.Safe.Color,
    Flag = "SafeColor",
    Callback = function(Value) LootSettings.Safe.Color = Value end,
})

local MilSection = WorldTab:CreateSection("Military Crates (Военные ящики)")

WorldTab:CreateToggle({
    Name = "Enable Military ESP",
    CurrentValue = false,
    Flag = "MilEnabled",
    Callback = function(Value) LootSettings.MilCrate.En = Value end,
})

WorldTab:CreateToggle({
    Name = "Show Military Name",
    CurrentValue = false,
    Flag = "MilName",
    Callback = function(Value) LootSettings.MilCrate.Nm = Value end,
})

WorldTab:CreateColorPicker({
    Name = "Military Color",
    Color = LootSettings.MilCrate.Color,
    Flag = "MilColor",
    Callback = function(Value) LootSettings.MilCrate.Color = Value end,
})
...
WorldTab:CreateSlider({
    ...
    CurrentValue = 1500,
    Flag = "LootDist",
    Callback = function(Value) LootSettings.MaxDist = Value end,
})

-- Функция отрисовки (добавь её в конец скрипта, где у тебя циклы)
local function UpdateLootESP(Model, Config, LabelText, PrimaryPartName)
    local TargetPart = Model:FindFirstChild(PrimaryPartName) or Model:FindFirstChildOfClass("BasePart")
    if not TargetPart then return end

    local Dist = (camera.CFrame.Position - TargetPart.Position).Magnitude

    if Dist > LootSettings.MaxDist or not Config.En then
        if Model:FindFirstChild("E_HL") then Model.E_HL:Destroy() end
        if Model:FindFirstChild("E_Gui") then Model.E_Gui:Destroy() end
        return
    end

    -- Highlight
    local HL = Model:FindFirstChild("E_HL") or Instance.new("Highlight", Model)
    HL.Name = "E_HL"
    HL.FillColor = Config.Color
    HL.FillTransparency = 0.5
    ...
    -- Billboard
    if Config.Nm then
        local Gui = Model:FindFirstChild("E_Gui") or Instance.new("BillboardGui", Model)
        Gui.Name = "E_Gui"
        Gui.AlwaysOnTop = true
        Gui.Size = UDim2.new(0, 50, 0, 25)
        Gui.ExtentsOffset = Vector3.new(0, 1, 0)

        local Label = Gui:FindFirstChild("L") or Instance.new("TextLabel", Gui)
        Label.Name = "L"
        ...
        Label.TextSize = 12
        Label.Text = string.format("%s\n[%dm]", LabelText, math.floor(Dist))
    elseif Model:FindFirstChild("E_Gui") then
        Model.E_Gui:Destroy()
    end
end

-- Основной цикл для лута
task.spawn(function()
    while task.wait(0.2) do
        for _, obj in pairs(workspace:GetChildren()) do
            if obj.Name == "Model" then
                -- Логика для Сейфа
                if obj:FindFirstChild("Wheel") and obj:FindFirstChild("Bolts") then
                    UpdateLootESP(obj, LootSettings.Safe, "SAFE", "Body")
                -- Логика для Военного ящика
                elseif obj:FindFirstChild("Bottom") and obj:FindFirstChild("Handles") then
                    UpdateLootESP(obj, LootSettings.MilCrate, "MILITARY CRATE", "Bottom")
                end
            end
        end
    end
